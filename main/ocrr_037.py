...
class OCRApp(tk.Frame):
    def __init__(self, master=None):
        ...
        self.batch_folder_path = None  # Add this line
...
class OCRApp(tk.Frame):
    ...
    def init_ui(self):
        ...
        frame = tk.Frame(self.master, height=50)
        frame.pack()

        batch_folder_button = tk.Button(self.master, text='Select batch folder', command=self.select_batch_folder)
        batch_folder_button.pack()

        batch_convert_button = tk.Button(self.master, text='Batch Convert', command=self.batch_convert)
        batch_convert_button.pack()
    ...
    def select_batch_folder(self):
        ...
        if folder_path:
            ...
            # Get the names of all image files in the selected folder
            image_file_names = [f for f in os.listdir(self.batch_folder_path) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp'))]

            # Display the names of the image files in the text edit widget
            if image_file_names:
                self.text_edit.delete('1.0', tk.END)
                self.text_edit.insert('1.0', '\n'.join(image_file_names))
            else:
                self.text_edit.delete('1.0', tk.END)
                self.text_edit.insert('1.0', 'No image files found in the selected folder.')
    ...
    def batch_convert(self):
        if self.batch_folder_path:  # Use self.batch_folder_path instead of folder_path
            file_names = []
            for file in os.listdir(self.batch_folder_path):
                if file.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
                    image_file = os.path.join(self.batch_folder_path, file)
                    text = pytesseract.image_to_string(image_file, lang=self.lang_var.get())
                    save_file = os.path.join(self.batch_folder_path, f'{os.path.splitext(file)[0]}.txt')
                    with open(save_file, 'w', encoding='utf-8') as f:
                        f.write(text)
                    file_names.append(file)

            messagebox.showinfo('알림', '배치 변환 완료!')
            
            # Display the names of the processed files in the text edit widget
            if file_names:
                self.text_edit.delete('1.0', tk.END)
                self.text_edit.insert('1.0', '\n'.join(file_names))
            else:
